...
proc onFrame*(display: ptr GLFMDisplay; frameTime: cdouble){.cdecl.}
proc onSurfaceCreated*(display: ptr GLFMDisplay; width: cint; height: cint){.cdecl.}
proc onSurfaceDestroyed*(display: ptr GLFMDisplay) {.cdecl.}
...
proc glfmMain*(display: ptr GLFMDisplay) {.exportc.} =
  ...
  discard glfmSetSurfaceCreatedFunc(display, onSurfaceCreated)
  discard glfmSetSurfaceResizedFunc(display, onSurfaceCreated)
  discard glfmSetSurfaceDestroyedFunc(display, onSurfaceDestroyed)
  discard glfmSetMainLoopFunc(display, onFrame)
  discard glfmSetTouchFunc(display, onTouch)
  discard glfmSetKeyFunc(display, onKey)
...
proc onFrame*(display: ptr GLFMDisplay; frameTime: cdouble) {.exportc.} =
  ...
  if app.program == 0:
    echo "setup"
    var vertShader: GLuint = compileShader(GL_VERTEX_SHADER, """
...
""")
    var fragShader: GLuint = compileShader(GL_FRAGMENT_SHADER, """
varying lowp vec3 v_color;
...
""")
    if vertShader == 0 or fragShader == 0:
      discard glfmSetMainLoopFunc(display, nil)
      return
